...
function goBack() {
  if (viewStack.length > 1) { viewStack.pop(); const p = viewStack[viewStack.length-1]; navigate(p.view, p.data, false); }
}
function navigate(view, data={}, push=true) {
  curState = { view, ...data }; editingPostId = null;
...
async function renderForum() {
  viewStack = [{ view: 'forum', data: {} }]; curState.view = 'forum';
  ...
  const stats = '<div class="stats-bar"><span>&#128196; Threads: ' + (threads||[]).length + '</span><span>&#128172; Posts: ' + (allPosts||[]).length + '</span><span>&#128101; ' + (currentProfile ? esc(currentProfile.username) + ' - welcome back!!!' : 'browsing as Guest') + '</span></div>';
...
async function renderThread(threadId) {
  ...
  // unique view: only count if logged in AND not the thread author
  ...
  const replyBox = currentUser
    ? '<div class="win-outer"><div class="win-title"><span>&#9997; Post a Reply</span></div><div style="margin:6px"><div style="margin-bottom:4px;font-size:11px">Posting as: <b>' + esc(currentProfile?.username||'') + '</b></div><textarea id="reply-text" style="width:100%;height:90px" placeholder="type ur reply xD rawr!!!"></textarea><div style="margin-top:5px;display:flex;gap:6px"><button class="btn98 btn-primary" onclick="submitReply(' + threadId + ')">Post Reply!!!</button><button class="btn98" onclick="navigate('forum')">Cancel</button></div><div id="reply-msg" style="margin-top:4px"></div></div></div>'
...
(async () => { await initAuth(); await renderForum(); })();
